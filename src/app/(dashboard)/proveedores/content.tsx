...
export default function ProveedoresContent() {
    ...
    const confirmDelete = async () => {
        ...
        setProcesando(true);
        const res = await eliminarProveedor(proveedorAEliminar);
        ...
        <div className="p-4 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
                <div className="bg-white p-3 md:p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                        <p className="text-[10px] md:text-sm font-medium text-gray-500">Total Proveedores</p>
                        <p className="text-xl md:text-2xl font-bold text-gray-900">{proveedores.length}</p>
                    </div>
                </div>
            ...
            <DeleteConfirmationModal
                ...
                description="Esta acción eliminará el proveedor del sistema. ¿Está seguro que desea continuar?"
                loading={procesando}
            />
        </div>
    );
